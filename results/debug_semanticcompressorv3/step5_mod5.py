from typing import Dict, List, Optional

class Transaction:
    def __init__(self, tx_id: int, description: str, amount: float, date: str, category: str = "기타"):
        self.tx_id = tx_id
        self.description = description
        self.amount = amount
        self.date = date
        self.category = category
        self.is_cancelled = False

    def cancel(self):
        self.is_cancelled = True

class BudgetTracker:
    def __init__(self):
        self.transactions: Dict[int, Transaction] = {}
        self.archived_transactions: Dict[int, Transaction] = {}
        self.budget_limits: Dict[str, float] = {}

    def set_budget_limit(self, category: str, limit: float) -> None:
        self.budget_limits[category] = limit

    def get_budget_status(self, category: str) -> Optional[Dict]:
        if category not in self.budget_limits:
            return None
        total_spent = sum(transaction.amount for transaction in self.transactions.values() if transaction.category == category and not transaction.is_cancelled)
        remaining_limit = self.budget_limits[category] - total_spent
        return {
            # ... (생략)
        }

    def add_transaction(self, tx_id: int, description: str, amount: float, date: str, category: str = "기타") -> None:
        if tx_id in self.transactions:
            raise ValueError("Transaction ID already exists")
        new_transaction = Transaction(tx_id, description, amount, date, category)
        self.transactions[tx_id] = new_transaction
        if category in self.budget_limits and abs(amount) > self.budget_limits[category]:
            print(f"Warning: Exceeding budget limit for {category}. Amount: {amount}")
            # raise ValueError("Transaction exceeds budget limit")

    def get_transaction(self, tx_id: int) -> Optional[Transaction]:
        return self.transactions.get(tx_id)

    def cancel_transaction(self, tx_id: int) -> None:
        if tx_id not in self.transactions:
            raise ValueError("Transaction ID does not exist")
        transaction_to_cancel = self.transactions[tx_id]
        if transaction_to_cancel.is_cancelled:
            raise ValueError("Transaction is already cancelled")
        # ... (생략)
        cancellation_tx_id = f"{tx_id}_cancel"
        new_cancellation_transaction = Transaction(cancellation_tx_id, "Cancellation", -transaction_to_cancel.amount, transaction_to_cancel.date, transaction_to_cancel.category)
        self.transactions[cancellation_tx_id] = new_cancellation_transaction
        self.archived_transactions[tx_id] = self.transactions.pop(tx_id)

    def get_all_transactions(self) -> List[Transaction]:
        return list(self.transactions.values())

    def get_transactions_by_category(self, category: str) -> List[Transaction]:
        return [transaction for transaction in self.transactions.values() if transaction.category == category]

    def get_monthly_summary(self, year_month: str) -> float:
        month = int(year_month.split("-")[1])
        transactions_in_month = [
            transaction.amount
            for transaction in self.transactions.values()
            if transaction.date.startswith(f"{year_month}-") and not transaction.is_cancelled
        # ... (생략)
        return sum(transactions_in_month)

    def update_transaction(self, tx_id: int, **kwargs) -> None:
        if tx_id not in self.transactions:
            raise ValueError("Transaction ID does not exist")
        transaction = self.transactions[tx_id]
        for key, value in kwargs.items():
            if hasattr(transaction, key):
                setattr(transaction, key, value)

    def archive_old_transactions(self, before_date: str) -> None:
        old_transactions = {tx_id: transaction for tx_id, transaction in self.transactions.items() if transaction.date < before_date}
        for tx_id, transaction in old_transactions.items():
            cancellation_tx_id = f"{tx_id}_cancel"
            new_cancellation_transaction = Transaction(cancellation_tx_id, "Cancellation", -transaction.amount, transaction.date, transaction.category)
            self.archived_transactions[tx_id] = self.transactions.pop(tx_id)
            self.transactions[cancellation_tx_id] = new_cancellation_transaction

    def get_archived_transactions(self) -> List[Transaction]:
        return list(self.archived_transactions.values())