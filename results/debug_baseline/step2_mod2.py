...
if __name__ == "__main__":
    ...
    for order in manager.list_orders():
        print(f"ID: {order.order_id}, Items: {[f'{item.name} x{item.quantity}' for item in order.items]}, Discount: {order.discount_percent*100}%, Total: {order.total}")

    order = manager.get_order(1)
    if order:
        print(f"\nRetrieved Order ID 1: Items - {[f'{item.name} x{item.quantity}' for item in order.items]}, Discount - {order.discount_percent*100}%, Total - {order.total}")
    
    manager.apply_discount(1, 0.1)
    print("\nAfter Applying 10% Discount to Order ID 1:")
    for order in manager.list_orders():
        print(f"ID: {order.order_id}, Items: {[f'{item.name} x{item.quantity}' for item in order.items]}, Discount: {order.discount_percent*100}%, Total: {order.total}")
