...
class BudgetTracker:
    def __init__(self):
        ...
        self.archived_transactions: List[Transaction] = []
    ...
    def get_archived_transactions(self) -> List[Transaction]:
        return self.archived_transactions

    def archive_old_transactions(self, before_date: str) -> None:
        for tx in self.transactions[:]:
            if tx.date < before_date:
                self.archived_transactions.append(tx)
...
tracker.archive_old_transactions("2023-04-15")
print("\nArchived Transactions:")
for tx in tracker.get_archived_transactions():
    print(f"ID: {tx.tx_id}, Description: {tx.description}, Amount: {tx.amount}, Date: {tx.date}, Category: {tx.category}")

# Get archived transactions example
print("\nAll Transactions (including archived):")
for tx in tracker.get_all_transactions() + tracker.get_archived_transactions():
    print(f"ID: {tx.tx_id}, Description: {tx.description}, Amount: {tx.amount}, Date: {tx.date}, Category: {tx.category}")