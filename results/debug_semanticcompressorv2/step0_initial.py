...
class BudgetTracker:
    ...
    def add_transaction(self, tx_id: int, description: str, amount: float, date: str) -> None:
        ...
        new_tx = Transaction(tx_id, description, amount, date)
    ...
    def cancel_transaction(self, tx_id: int) -> None:
        ...
        new_tx = Transaction(tx_id, tx.description, -tx.amount, tx.date)  # Cancel by adding a negative transaction
        new_tx.cancel()
        self.transactions.append(new_tx)
...
tracker.add_transaction(1, "Groceries", 50.0, "2023-04-01")
tracker.add_transaction(2, "Salary", 1000.0, "2023-04-01")

print("All Transactions:")
for tx in tracker.get_all_transactions():
    print(f"ID: {tx.tx_id}, Description: {tx.description}, Amount: {tx.amount}, Date: {tx.date}")
