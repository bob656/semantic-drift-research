...
class Order:
    def __init__(self, order_id, items, inventory):
...
class OrderManager:
    ...
    def add_order(self, order_id: int, items: list) -> None:
        ...
        order = Order(order_id, items, self.inventory)
...
try:
    ...
    print(f"Order 1: {order1.order_id}, Items: {[item.name for item in order1.items]}, Total: {order1.total}, Status: {order1.status}, Created At: {order1.created_at}")
    ...
    print(f"Order History: {[order.order_id for order in order_history]}")

    pending_orders = order_manager.get_orders_by_status("PENDING")
    print(f"Pending Orders: {[order.order_id for order in pending_orders]}")
