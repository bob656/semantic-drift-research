...
class Order:
    ...
    def __repr__(self):
        ...
        return f"Order(order_id={self.order_id}, items={self.items}, total={self.total}, discount_percent={self.discount_percent}, status={self.status}, created_at={self.created_at})"
...
class OrderManager:
    ...
    def add_order(self, order_id, items, inventory):
        ...
        print(f"주문 {order_id}이(가) 생성되었습니다.")
    ...
    def process_payment(self, order_id, amount, method):
        ...
        if order.status != "PENDING":
            raise ValueError(f"주문 {order_id}은 이미 처리되었습니다.")
        ...
        print(f"결제 {payment_id}이(가) 처리되었습니다.")
    ...
    def refund_payment(self, order_id):
        ...
        if order.status == "REFUNDED":
            raise ValueError(f"주문 {order_id}은 이미 환불되었습니다.")
        ...
        if not payment:
            raise ValueError(f"주문 {order_id}에 대한 결제를 찾을 수 없습니다.")

        if payment.refunded:
            raise ValueError(f"결제 {payment.payment_id}은 이미 환불되었습니다.")

        payment.refunded = True
        order.status = "REFUNDED"
        print(f"주문 {order_id}의 결제가 환불되었습니다.")
