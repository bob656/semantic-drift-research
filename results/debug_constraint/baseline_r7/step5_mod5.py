...
class LoanSystem:
    ...
    def generate_monthly_report(self, year: int, month: int):
        ...
        for loan in self.loans.values():
            ...
            overdue_days = (current_date - last_payment_date).days if last_payment_date else 30
    ...
    def summarize_portfolio(self):
        ...
        for loan in self.loans.values():
            if loan.status == 'active':
                ...
                total_inactive_loans += 1
            
            total_principal += loan.principal
            total_paid_amount += sum(p.amount for p in loan.payments)

        average_interest_rate = INTEREST_RATE

        return {
            'total_active_loans': total_active_loans,
            'total_inactive_loans': total_inactive_loans,
            'total_principal': total_principal,
