...
class LoanSystem:
    ...
    def get_overdue_amount(self, loan_id: int) -> float:
        ...
        loan = self.loans[loan_id]
        overdue_months = len([p for p in loan.payments if p.type == 'REGULAR' and self.check_overdue(loan.loan_id, p.date, datetime.now().strftime("%Y-%m-%d"))])
        remaining_balance = loan.principal - sum(p.amount for p in loan.payments if p.type == 'REGULAR')
        overdue_interest = remaining_balance * (INTEREST_RATE / 12) * overdue_months
        late_fee_total = LATE_FEE * overdue_months
        
        return remaining_balance + overdue_interest + late_fee_total
...
overdue_status = loan_system.check_overdue(1, "2023-09-01", datetime.now().strftime("%Y-%m-%d"))
overdue_amount = loan_system.get_overdue_amount(1)
print(f"Overdue Status: {overdue_status}, Overdue Amount: {overdue_amount}")
...
# Generate monthly report and summarize portfolio
