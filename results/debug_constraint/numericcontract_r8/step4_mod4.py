...
if __name__ == "__main__":
    ...
    print(f"Loan 1 is overdue: {overdue_status}")

    if overdue_status:
        overdue_amount = system.get_overdue_amount(1)
        print(f"Overdue amount for loan 1: {overdue_amount}원")

    # 분할 납부 계획 생성
    payment_schedule = system.generate_payment_schedule(1)
    for schedule in payment_schedule:
        print(f"Month: {schedule['month']}, Payment Amount: {schedule['payment_amount']}원, Principal: {schedule['principal']}원, Interest: {schedule['interest']}원, Balance: {schedule['balance']}원")

    # 납부 완료 월 수 기준 잔액 계산
    remaining_balance = system.get_remaining_balance(1, 6)
    print(f"Remaining balance after 6 months of payments: {remaining_balance}원")