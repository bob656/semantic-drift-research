...
class LoanSystem:
    ...
    def calculate_early_repayment(self, loan_id: int, remaining_balance: float, remaining_months: int) -> Optional[float]:
        loan = self.loans.get(loan_id)
        if not loan or loan.status != 'active':
            return None
    ...
    def get_payoff_amount(self, loan_id: int) -> Optional[float]:
        ...
        monthly_interest_rate = INTEREST_RATE / 12
        total_interest = remaining_balance * monthly_interest_rate * (loan.months - len(loan.payments))
        payoff_amount = remaining_balance + total_interest
        return payoff_amount

    def check_overdue(self, loan_id: int, last_payment_date, today):
    ...
    def get_overdue_amount(self, loan_id: int):
        ...
        if not loan or loan.status != 'active':
            return None
        overdue_amount = loan.principal + LATE_FEE + (loan.principal * PENALTY_RATE)  # 연체료와 가산이자를 계산하여 추가
