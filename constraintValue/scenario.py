...
""",
    ...
    'modifications': [
        # 수정1: 납부 이력 — 수치 안내 없음 (Baseline 불리하게)
        """납부 이력 관리 기능을 추가하세요.
...
완전한 Python 코드를 반환하세요.""",
        ...
        # 수정2: 조기 상환 할인 — 수치 안내 없음
        """조기 상환 할인 계산 기능을 추가하세요.

LoanSystem에 메서드 추가:
- calculate_early_repayment(loan_id, remaining_balance): 조기 상환 시 절감 이자 계산
  * 잔여 이자 = remaining_balance * 월이율 * remaining_months
  * 조기 상환 할인액 = 잔여 이자 * 0.1 (10% 할인)
...
LoanSystem에 메서드 추가:
- check_overdue(loan_id, last_payment_date, today): 30일 이상 납부 없으면 연체 판정
- get_overdue_amount(loan_id): 연체 총액 반환 (미납 원금 + 연체료 + 가산 이자)

완전한 Python 코드를 반환하세요.""",

        # 수정4: 분할 납부 계획 — 수치 안내 없음
...
- generate_monthly_report(year, month): 해당 월 대출 현황 딕셔너리 반환
  * total_loans, total_principal, average_rate, overdue_count, total_late_fees
